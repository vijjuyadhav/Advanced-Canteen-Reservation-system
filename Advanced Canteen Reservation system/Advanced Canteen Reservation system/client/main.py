import secrets
import string
from database.func import add,list_recipes,key_info,update_recipe_stock,make_reservation,passgen,cancel_reservation
...
layout.append([sg.Button('Make Reservation',button_color="blue")])
layout.append([sg.Button('Cancel Reservation',button_color="red")])
...
while True:
    ...
    if event == "Make Reservation":
     
        lst = {x: int(values[x]) for x in plain_receipes if values[x] != ''}
        is_parcel = values["is_parcel"]
        
        if lst:  # Check if there are items in the list
            reservation_key = make_reservation(lst, is_parcel)
            sg.popup(f"Reservation made with key: {reservation_key}")
        
            for x in plain_receipes:
                window[x].update('')
            window['-ORDER-'].update('')
        else:
            sg.popup("Please select at least one item to make a reservation.")


    if event == "Cancel Reservation":
        reservation_key = values['reservation_key']  # Get the reservation key from the input field
        if reservation_key:  # Check if the key is not empty
            success = cancel_reservation(reservation_key)
            if success:
                sg.popup(f"Reservation {reservation_key} has been successfully canceled.")
                window['reservation_key'].update('')  # Clear the reservation key input
            else:
                sg.popup(f"Failed to cancel reservation or reservation {reservation_key} does not exist.")
        else:
            sg.popup("Please enter a reservation key to cancel.")